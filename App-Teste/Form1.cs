...
namespace App_Teste
{
    public partial class Flanges : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            double DmenorFuro = 0;
            double resultado = 0;
            for (int i = 0; i < Dfuracao.Count; i++)
            {
            ...
            Result.Text = "Folga de: " + resultado + "MM";
        }
        private void TesteJSON()
        {
            ...
            using(StreamReader reader = new StreamReader(@"c:\Users\joão vitor\source\repos\App-teste\App-teste\JSON\dados.json"))
            {
                JsonString = reader.ReadToEnd();
            }
            JObject json = JObject.Parse(JsonString);
            norma.Add(json.ToString());

        }

        private void Flanges_Load(object sender, EventArgs e)
        {
            var conn = Connection.Connection.connectionSLite();
        }
    }
